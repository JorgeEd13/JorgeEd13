...
- 🎲 **Reproducible by construction** — same config + seed → **byte-identical** output (one seeded generator spawned per unit per stage); **132 offline tests**, ADR-documented, CI green. A future MLOps project will consume it as its data source.
...
- 🧭 **Real-world hardening** — navigated MLflow 3's retired file-store (migrated tracking + registry to local SQLite without giving up server-free), and a guard that **fails loud on an undefined metric** instead of logging a silent `nan`. **49 offline tests**, ADR-documented.
- 🚧 **In progress (honest status)** — F0–F2.5 shipped (data, leakage-safe features, train + track + registry, ground-truth-scored outlier robustness). Next: HPO + diagnostics on the cleaned inputs, then **FastAPI serving** and the headline **drift → auto-retrain loop** (Evidently + Prefect, scheduled on free cloud runners). Nothing here claims a live production deployment.

`Python · MLflow · scikit-learn · LightGBM · PyTorch · SQLite · pandas · pytest` *(roadmap: FastAPI · Evidently · Prefect · GitHub Actions)*
