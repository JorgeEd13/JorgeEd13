...
- 🧭 **Real-world hardening** — navigated MLflow 3's retired file-store (migrated tracking + registry to local SQLite without giving up server-free), and a guard that **fails loud on an undefined metric** instead of logging a silent `nan`. **86 offline tests**, ADR-documented.
- 🚧 **In progress (honest status)** — F0–F2.8 shipped: the full **modelling arc is closed by design** (data, leakage-safe features, train + track + registry, ground-truth-scored outlier robustness, grouped-CV HPO + diagnostics + watchers, temporal modelling, and a measured ceiling characterization). Next is the **MLOps spine**: gated registry promotion, then **FastAPI serving** and the headline **drift → auto-retrain loop** (Evidently + Prefect, scheduled on free cloud runners). Nothing here claims a live production deployment.
