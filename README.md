...
- ✅ **A complete production spine (honest status)** — F0–F2.8 + **F3 + F4 + F5 + F6** shipped (roadmap **F0–F6 complete**): the **modelling arc is closed by design** (data, leakage-safe features, train + track + registry, ground-truth-scored outlier robustness, grouped-CV HPO + diagnostics + watchers, temporal modelling, and a measured ceiling characterization), and the **production spine runs end to end and is now reachable** — **train → registry → serve → drift → retrain → cloud-scheduled**, with a **live `/health`** you can click: gated promotion + rollback, FastAPI serving of the promoted model, the drift → auto-retrain loop that routes every retrain through the same gate, and a hosted free-tier deploy. Nothing here claims a live *production* deployment — the hosted model is a labelled demo; the loop is a demonstrated closed loop on synthetic data.

`Python · MLflow · scikit-learn · LightGBM · Optuna · PyTorch · Evidently · Prefect · FastAPI · Docker · SQLite · pandas · pytest` *(hosted free-tier deploy — a live /health link)*
