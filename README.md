...
- 🧭 **Real-world hardening** — navigated MLflow 3's retired file-store (migrated tracking + registry to local SQLite without giving up server-free), built promotion on **aliases not the deprecated stages**, and a guard that **fails loud on an undefined metric** instead of logging a silent `nan`. **100 offline tests**, ADR-documented.
- 🚧 **In progress (honest status)** — F0–F2.8 + **F3** shipped: the **modelling arc is closed by design** (data, leakage-safe features, train + track + registry, ground-truth-scored outlier robustness, grouped-CV HPO + diagnostics + watchers, temporal modelling, and a measured ceiling characterization), and the **production spine has started** with **gated registry promotion + rollback**. Next is **FastAPI serving** (F4), then the headline **drift → auto-retrain loop** (F5 — Evidently + Prefect, scheduled on free cloud runners). Nothing here claims a live production deployment.
